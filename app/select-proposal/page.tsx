...
import { proposalSlugs, proposals } from "@/proposals/registry";
    ...
    <main className="flex min-h-screen items-center justify-center px-6 py-12">
      <div className="w-full max-w-md">
        ...
        <p className="prose-body mb-8">
          No proposal is active. Pick one below, or set{" "}
          ...
          {proposalSlugs.map((slug) => {
            ...
            return (
              <li key={slug}>
                ...
                >
                  <p className="font-medium text-foreground">{proposal.hero.clientName}</p>
                  <p className="mt-0.5 text-[14px] text-secondary">{proposal.hero.title}</p>
        ...
        <p className="prose-body mt-6 text-[13px] text-secondary">
          ...
          <code className="font-mono text-foreground">PROPOSAL_SLUG=pmi</code> (or whichever
          client you&apos;re working on).
        </p>
      </div>
    </main>
  );
